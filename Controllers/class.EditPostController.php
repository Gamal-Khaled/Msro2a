...
class EditPostController extends PageController
{
        ...
        elseif ($_POST) {
            ...
            if(isset($_POST["postId"])){
                ...
                if(!$error){
                    ...
                    if((int)$_POST["postId"] == -1){
                        $tempPost = new Post(-1, $_POST["desc"], "imgs/client.jpg", $this -> currentUser, $_POST["name"]);
                        foreach (array_merge($questions, $newQuestions) as $q) {
                            $tempPost -> addNewQuestion($q);
                        }
                        foreach (array_merge($categories, $newCats) as $c) {
                            $tempPost -> addCategory($c);
                        }

                        $this -> postsSQLClient -> saveNewPost($tempPost, $this -> currentUser -> getId());
                        header("Location: index.php");
                    }
                }
            }
    ...
    public function onEditDescriptionClick()
    {
        
    }

    public function onEditquestions()
    {
        
    }

    public function onDeletePostClick()
    {
